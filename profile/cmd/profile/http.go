...
import (
	...
	"net/url"
	"os"
	profilesvr "profile/gen/http/profile/server"
...
func handleHTTPServer(ctx context.Context, u *url.URL, profileEndpoints *profile.Endpoints, wg *sync.WaitGroup, errc chan error, logger *log.Logger, debug bool) {
	...
	// responses.
	var (
		profileServer *profilesvr.Server
	)
	{
		eh := errorHandler(logger)
		profileServer = profilesvr.New(profileEndpoints, mux, dec, enc, eh, nil, nil)
		if debug {
	...
	// Configure the mux.
	profilesvr.Mount(mux, profileServer)
