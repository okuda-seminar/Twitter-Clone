...
describe("RabbitMqLogger", () => {
  ...
  it("should send a tap log to the logs queue", async () => {
    const objectId = "98765";

    await expect(logger.sendTapLog(objectId)).resolves.not.toThrow();
  ...
  it("should send multiple logs independently", async () => {
    await expect(logger.sendTapLog("12345")).resolves.not.toThrow();
  ...
  it("should throw an error if RabbitMQ is unreachable", async () => {
    ...
    await expect(errorLogger.sendTapLog("12345")).rejects.toThrow(
      "Connection failed",
    );
  });
});
